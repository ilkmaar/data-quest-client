...
const { ASSIGN_PLAYER_TO_USER, ADD_USER_TO_WORLD, REMOVE_USER_FROM_WORLD } =
  adminMutations;
...
const actions = {
  async fetchData(
    { commit },
    { query, mutationName, commitType, options = {} }
  ) {
    commit("SET_LOADING", true);
    commit("SET_ERROR", null);

    try {
      const { result, error, run, refetch } = useDefinedQuery(
        query,
        options.variables || {},
        options
      );
  ...
  async modifyUserPlayers(
    { commit, dispatch },
    { mutation, userId, playerId }
  ) {
    commit("SET_LOADING", true);
    commit("SET_ERROR", null);
  ...
  assignPlayerToUser({ dispatch }, payload) {
    return dispatch("modifyUserPlayers", {
      ...payload,
      mutation: ASSIGN_PLAYER_TO_USER,
    });
  },

  addUserToWorld({ dispatch }, payload) {
    return dispatch("modifyUserWorld", {
      ...payload,
      mutation: ADD_USER_TO_WORLD,
    });
  },
  ...
  removeUserFromWorld({ dispatch }, payload) {
    return dispatch("modifyUserWorld", {
      ...payload,
      mutation: REMOVE_USER_FROM_WORLD,
    });
  },
};
